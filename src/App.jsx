...
const PricingCalculator = () => {
  ...
  const calculateSLPricing = () => {
    ...
    const other = otherFee + (gpv * 0.05); // used only for display of the "Other" comparison row
  ...
  const renderSingleLocationTab = () => {
    ...
    // FIX: currentPrice for "Other" must use raw slOtherSubFee, not pricing.other
    // pricing.other adds gpv * 0.05 on top of the sub fee, which inflated savings
            ...
            {slScenario === 'new' ? (
              ...
              <div className="space-y-4">
                {[
                  ...
                  // FIX: use currentPrice defined above (raw sub fee for Other, not sub fee + CX fee)
                        ...
                        <div className="text-right">
                          ...
                          {slCurrentModel && !isCurrent && (
                            <p className={`text-sm font-medium ${savings > 0 ? 'text-green-600' : savings < 0 ? 'text-red-600' : 'text-gray-500'}`}>
                              {savings > 0 ? `Save ${formatCurrency(savings)}/mo` : savings < 0 ? `+${formatCurrency(Math.abs(savings))}/mo more` : 'Same price'}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                          ...
                          {mlCurrentSubFee && !option.isCurrent && (
                            <p className={`text-sm font-medium ${savings > 0 ? 'text-green-600' : savings < 0 ? 'text-red-600' : 'text-gray-500'}`}>
                              {savings > 0 ? `Save ${formatCurrency(savings)}/loc/mo` : savings < 0 ? `+${formatCurrency(Math.abs(savings))}/loc/mo more` : 'Same price'}
                            </p>
                          )}
                        </div>
                      </div>
